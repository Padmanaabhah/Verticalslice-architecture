...
{ 
    var filteredDestinations = destinations.Where(d => 
        searchFor == null ||
        d.Name.Contains(searchFor, StringComparison.OrdinalIgnoreCase) || 
        (d.Description != null && d.Description.Contains(searchFor, StringComparison.OrdinalIgnoreCase)));
    Task.Delay(5000).Wait(); // simulate some latency
